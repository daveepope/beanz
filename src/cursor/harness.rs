...
use super::transcript::{edit_ops_from_line, parse_line, read_est_chars_from_session};
...
impl CursorHarness {
    pub fn new(path: PathBuf, workspace_root: PathBuf) -> Self {
        Self {
            engine: SessionEngine::new(path, parse_line, edit_ops_from_line),
            complexity: ComplexityEngine::new(workspace_root.clone()),
            workspace_root,
...
impl Harness for CursorHarness {
    ...
    fn features(&self) -> Features {
        ...
        features.read_est_chars = read_est_chars_from_session(
            self.engine.path(),
            &self.workspace_root,
        )
        .unwrap_or(0);
